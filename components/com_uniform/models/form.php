...
class JSNUniformModelForm extends JModelItem
{
	...
	public function save($post)
	{
		...
		//$pageTitle = $postData['page_title'] ? $postData['page_title'] : $document->getTitle(); 
		
		//Run Script on Form Process
		$formScriptOnProcess = trim($dataForms->form_script_on_process);
		if ($formScriptOnProcess != '')
		{
			if (version_compare(PHP_VERSION, '7.0.0', '<'))
			{
				try
				{
					eval($formScriptOnProcess);
				}
				catch (Exception $e)
				{
		...
		foreach ($columsSubmission as $colum)
		{
			if (!in_array($colum->field_id, $fieldClear))
			{
				...
				if ($colum->field_type != 'static-content' && $colum->field_type != 'google-maps')
				{
					...
					//elseif ($colum->field_type == "page_title")
					//{
					//	$value = $pageTitle;
					//}
...
//		$regex = '/^([\p{L}\.\-\d]+)@([\p{L}\-\.\d]+)(\.[\p{L}])((\.[\p{L}]{2,63})+)$/';
		if (!empty($listEmail) && is_array($listEmail) && count($listEmail))
		{
			...
			/*$templateFrom = trim($dataTemplates->template_from);
			
			if (!empty($templateFrom))
			{
				$templateFrom = strtolower($templateFrom);
			...
			//$mailfrom = $app->getCfg('mailfrom');
			$fromname = empty($dataTemplates->template_from) ? $jconfig->get('fromname') : $dataTemplates->template_from;
