	...
	if ($config_data->joomla_captcha != 0)
        ...
        if ($global_config_captcha != '0')
            ...
            if (is_object($captcha_plugin))
                if ($captcha_plugin->checkAnswer('') === false)
...
// build the Joomla mail object
	...
	$this->data->admin_from_email = $app->getCfg('mailfrom'); // for the log model
	
	$mail->addRecipient($config_data->toPrimary);
	$this->data->admin_email = $config_data->toPrimary;		  // for the log model
	...
	$this->data->admin_reply_to_email = $this->data->from_email; // for the log model
...
// if we should send the user a copy, send it separately
	...
	$this->data->config_show_copy = $config_data->show_copy;	// for the log model
	$this->data->show_copy = $this->data->copy_me; 				// for the log model
	$this->data->user_from_email = '';							// for the log model
	...
	if (($config_data->show_copy == LAFC_COPYME_ALWAYS) or ($this->data->copy_me == 1))
		...
		$mail->setSender(array($app->getCfg('mailfrom'), $app->getCfg('fromname')));
		$this->data->user_from_email = $app->getCfg('mailfrom'); // for the log model
...
function email_check($config_data)
{
	...
	$lang->load(strtolower(LAFC_COMPONENT), JPATH_COMPONENT_ADMINISTRATOR);	// load the back end language file
