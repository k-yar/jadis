...
<div class="easy_entries">
	<?php foreach($this->entries as $entry) : ?>
		<div class="easy_frame"
		     <?php if(!$entry->published) : ?>style="background-color: #fffefd; border: #ffb39b solid 1px;"<?php endif; ?>>
			<div class="easy_top" <?php if(!$entry->published) : ?>style="background-color: #FFE7D7;"<?php endif; ?>>
				<div class="easy_top_left">
					<strong class="easy_big"
					        id="gbentry_<?php echo $entry->id; ?>"><?php echo $entry->gbname ?></strong>
					<strong class="easy_small">
						...
						<?php endif; ?>
						<?php if(!$entry->published) : ?>| <strong
							class="easy_small_red"><?php echo JText::_('COM_EASYBOOKRELOADED_ENTRY_OFFLINE'); ?></strong><?php endif; ?>
					</strong>
				</div>
				<div class="easy_top_right">
					<?php if($this->params->get('show_rating', true) AND $entry->gbvote !== '0') : ?>
						...
						<?php endfor; ?>
					<?php endif; ?>
					<?php if(EASYBOOK_CANEDIT) : ?>
						<div class="easy_top_right_admin">
							<a href="<?php echo JRoute::_('index.php?option=com_easybookreloaded&controller=entry&task=edit&cid='.(int)$entry->id); ?>"
							   title="<?php echo JText::_('COM_EASYBOOKRELOADED_EDIT_ENTRY'); ?>"><?php echo JHtml::_('image', 'components/com_easybookreloaded/images/edit.png', JText::_('COM_EASYBOOKRELOADED_EDIT_ENTRY'), 'class="easy_align_middle"'); ?></a>
							<a href="<?php echo JRoute::_('index.php?option=com_easybookreloaded&controller=entry&task=remove&cid='.(int)$entry->id); ?>"
							   title="<?php echo JText::_('COM_EASYBOOKRELOADED_DELETE_ENTRY'); ?>"><?php echo JHtml::_('image', 'components/com_easybookreloaded/images/delete.png', JText::_('COM_EASYBOOKRELOADED_DELETE_ENTRY'), 'class="easy_align_middle"'); ?></a>
							<?php if(empty($entry->gbcomment)) : ?>
								<a href="<?php echo JRoute::_('index.php?option=com_easybookreloaded&controller=entry&task=comment&cid='.(int)$entry->id); ?>"
								   title="<?php echo JText::_('COM_EASYBOOKRELOADED_ADD_COMMENT'); ?>"><?php echo JHtml::_('image', 'components/com_easybookreloaded/images/comment.png', JText::_('COM_EASYBOOKRELOADED_ADD_COMMENT'), 'class="easy_align_middle"'); ?></a>
							<?php else : ?>
								<a href="<?php echo JRoute::_('index.php?option=com_easybookreloaded&controller=entry&task=comment&cid='.(int)$entry->id); ?>"
								   title="<?php echo JText::_('COM_EASYBOOKRELOADED_EDIT_COMMENT'); ?>"><?php echo JHtml::_('image', 'components/com_easybookreloaded/images/comment_edit.png', JText::_('COM_EASYBOOKRELOADED_EDIT_COMMENT'), 'class="easy_align_middle"'); ?></a>
							<?php endif; ?>
							<?php if(empty($entry->published)) : ?>
								<a href="<?php echo JRoute::_('index.php?option=com_easybookreloaded&controller=entry&task=publish&cid='.(int)$entry->id); ?>"
								   title="<?php echo JText::_('COM_EASYBOOKRELOADED_PUBLISH_ENTRY'); ?>"><?php echo JHtml::_('image', 'components/com_easybookreloaded/images/offline.png', JText::_('COM_EASYBOOKRELOADED_PUBLISH_ENTRY'), 'class="easy_align_middle"'); ?></a>
							<?php else : ?>
								<a href="<?php echo JRoute::_('index.php?option=com_easybookreloaded&controller=entry&task=publish&cid='.(int)$entry->id); ?>"
								   title="<?php echo JText::_('COM_EASYBOOKRELOADED_UNPUBLISH_ENTRY'); ?>"><?php echo JHtml::_('image', 'components/com_easybookreloaded/images/online.png', JText::_('COM_EASYBOOKRELOADED_UNPUBLISH_ENTRY'), 'class="easy_align_middle"'); ?></a>
							<?php endif; ?>
						</div>
					<?php endif; ?>
				</div>
			...
			<?php if((!empty($entry->gbmail) AND $entry->gbmailshow AND $this->params->get('show_mail', true)) OR (!empty($entry->gbpage) AND $this->params->get('show_home', true)) OR (!empty($entry->gbicq) AND $this->params->get('show_icq', true)) OR (!empty($entry->gbaim) AND $this->params->get('show_aim', true)) OR (!empty($entry->gbmsn) AND $this->params->get('show_msn', true)) OR (!empty($entry->gbyah) AND $this->params->get('show_yah', true)) OR (!empty($entry->gbskype) AND $this->params->get('show_skype', true))) : ?>
				<div class='easy_contact'>
					<?php if(!empty($entry->gbmail) AND $entry->gbmailshow AND $this->params->get('show_mail', true)) : ?>
						...
						<?php echo EasybookReloadedHelperContent::cloak($entry->gbmail, true, $image, false); ?>
					<?php endif; ?>
					<?php if(!empty($entry->gbpage) AND $this->params->get('show_home', true)) : ?>
						<a href="<?php echo $entry->gbpage; ?>"
						   title="<?php echo JText::_('COM_EASYBOOKRELOADED_HOMEPAGE').' - '.$entry->gbpage; ?>"
						   <?php if($this->params->get('nofollow_home', true)) : ?>rel="nofollow"
						   <?php endif; ?>target="_blank"><?php echo JHtml::_('image', 'components/com_easybookreloaded/images/world.png', $entry->gbpage, 'height="16" width="16" hspace="3"'); ?></a>
					<?php endif; ?>
			...
			<?php if($entry->gbcomment) : ?>
				<div
					class="easy_admincomment"><?php echo JHtml::_('image', 'components/com_easybookreloaded/images/admin.png', JText::_('COM_EASYBOOKRELOADED_ADMIN_COMMENT:'), 'class="easy_align_middle"'); ?>
					<strong>
