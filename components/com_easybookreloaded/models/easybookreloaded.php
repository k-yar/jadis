...
class EasybookReloadedModelEasybookReloaded extends JModelLegacy
{
	...
	private function buildQuery()
	{
		...
		$query = "SELECT * FROM ".$this->_db->quoteName('#__easybook')." WHERE ".$this->_db->quoteName('gbid')." = ".$this->gbid;
	...
	{
		$gb_data = $this->_getList("SELECT * FROM ".$this->_db->quoteName('#__easybook_gb')." WHERE ".$this->_db->quoteName('id')." = ".$this->gbid);
	...
	private function buildCountQuery()
	{
		...
		{
			$query = "SELECT * FROM ".$this->_db->quoteName('#__easybook')." WHERE ".$this->_db->quoteName('gbid')." = ".$this->gbid." AND ".$this->_db->quoteName('published')." = 1";
		}
	...
	{
		if(!empty($this->gbid) AND is_numeric($this->gbid))
		{
			return $this->gbid;
		}
	}
