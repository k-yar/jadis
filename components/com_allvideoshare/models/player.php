...
class AllVideoShareModelPlayer extends AllVideoShareModel {
	...
	function buildPlayer( $videoid = 1, $playerid = 1, $autodetect = 0 ) {
		 ...
		 if($video->type == 'thirdparty') {
			...
			} else if(preg_match('/msie 10/i', $_SERVER['HTTP_USER_AGENT']) || strpos($_SERVER['HTTP_USER_AGENT'], 'Trident/7.0') !== false) {
		    	$result = $this->gethtmlplayer( $video );
			} else {
	...
	function gethtmlplayer( $video ) {
		 if($video->type == 'youtube') {
			...
			$result .= 'src="https://www.youtube.com/embed/'.$args['v'].'?rel=0&showinfo=0&iv_load_policy=3&modestbranding=1" frameborder="0" allowfullscreen></iframe>';
			$result .= '</div>';
		 } else {
